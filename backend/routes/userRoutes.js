...
// Generate AES key and share between users
router.post('/connect', async (req, res) => {
    ...
    try {
        ...
        // Store AES key in activeChatKeys map
        senderUser.activeChatKeys.set(receiver, aesKey);
        receiverUser.activeChatKeys.set(sender, aesKey);

        await senderUser.save();
        await receiverUser.save();

        res.json({ message: "AES key generated and shared successfully.", aesKey });
    } catch (error) {
        console.error("Error generating AES key:", error);
        res.status(500).json({ message: "Failed to generate AES key.", error: error.message });
    }
});
...
router.post('/disconnect', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ message: "Failed to disconnect chat.", error: error.message });
    }
});
